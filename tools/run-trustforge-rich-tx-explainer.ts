...
export async function runRichTxExplainerPhase3(options: {
  ...
  if (!policy) {
    const status = gate.authorized ? "PASS_RICH_TARGET_READY_NO_ENDPOINT" : "PASS_RICH_TARGET_READY_NO_ENDPOINT";
  ...
  try {
    ...
    const status = "PASS_RICH_TARGET_READY_NO_ENDPOINT";
    const resultLines = buildResult({
      status,
      commitBefore,
      commitAfter: commitBefore,
  ...
  if (!options.executePaid || !gate.authorized) {
    ...
    await writeText(
      ...
      [
        ...
        "",
        `- status: ${gate.authorized ? "PASS_RICH_TARGET_READY_HUMAN_GATE" : "PASS_RICH_TARGET_READY_HUMAN_GATE"}`,
  ...
  try {
    ...
    const claims = extractTxExplainerClaims({ body: paidResponse.responseBody });
    ...
    const probeRun = {
      ...
      payment: {
        ...
        actual_amount_usdc: paidResponse.actualAmountUsdc,
...
function buildResult(input: {
  ...
  readonly onchainPayment?: { status: string };
  ...
  const next =
    input.status === "PASS_RICH_TX_EXPLAINER_SCORE"
      ...
      : input.status === "PASS_RICH_TARGET_READY_HUMAN_GATE"
        ? "Set the rich tx_explainer gate variables with an explicit cap and rerun this same spec; discovery, fixtures, verifiers, and tests are already ready."
  ...
  return [
    ...
    `payment_bearing_http_request_count: ${input.paymentAttempted ? 1 : 0}`,
    `retry_used: no`,
    `fallback_used: no`,
    `actual_spend_usdc: ${input.handshake?.quoteUsdc ?? "null"}`,
    ...
    `seller_response_captured: ${input.paymentAttempted ? "yes" : "no"}`,
    ...
    `probe_run_created: ${input.status === "PASS_RICH_TX_EXPLAINER_SCORE" ? "yes" : "no"}`,
    `evaluation_result_created: ${input.status === "PASS_RICH_TX_EXPLAINER_SCORE" ? "yes" : "no"}`,
    `trust_score_rich_created: ${input.status === "PASS_RICH_TX_EXPLAINER_SCORE" ? "yes" : "no"}`,
